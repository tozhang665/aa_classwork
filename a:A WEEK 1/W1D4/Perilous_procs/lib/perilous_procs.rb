...
################# PHASE 2 ############################
def xnor_select(arr , block1 , block2)
    arr2 = []
    arr.each do |val|
        if block1.call(val) == block2.call(val)
            arr2 << val
        end
    end
    arr2
...
def multi_map(array,num = 1,&block)
    mapped = []
    array.each do |val|
        num.times do 
            val = block.call(val)
...
def proctition_platinum(arr,*procArr)
    hash={}
    (1..procArr.length).each do |n|
        hash[n] = []
    end
    arr.each do |word| 
        procArr.each do |proc|
            if proc.call(word)
                hash[procArr.index(proc)+1] << word
                break
            end
        end
    end

...
def energetic_encoding(str,hash)
    ...
    words.each_with_index do |word,idx|
        ...
        word.each_char.with_index do |char,idx2|
            hash.has_key?(char) ? replace += hash[char] : replace += "?"
...
def uncompress(str)
    ...
    while idx < str.length
        str[idx+1].to_i.times{retStr +=str[idx]}
        idx +=2
    end
    retStr
end
...
def convert_pig_latin(str)
    ...
    words.each_with_index do |word,idx|
        if word.length >= 3
            ("aeiou").include?(word[0].downcase) ? words[idx] = word + "yay" : words[idx] = fixer(word)
        end
    end
    base.each_with_index do |bword,idx1|
        if bword[0] == bword[0].upcase
            words[idx1] = words[idx1].capitalize
        end
    end
    words.join(" ")
end
    ...
    words.each_with_index do |word,idx|
        if word.length >=3
            if ("aeiou").include?(word[-1])
                ...
                vowelsIdx = []
                word.each_char.with_index do |char,idx2|
                    vowelsIdx << idx2 if ("aeiou").include?(char)
                end
...
def disjunct_select(arr,*procs)
    my_arr = []
    arr.each do |ele|
        procs.each {|proc|  my_arr << ele if proc.call(ele)}
    end
    my_arr.uniq
end
...
def compress(str)
    ...
    str.each_char.with_index do |char,idx|
        if str[idx] == str[idx+1]
            ...
            count == 1 ? base = base + char : base = base + char + count.to_s
            # if count == 1
            #     base = base + char
            # else

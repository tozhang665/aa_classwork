...
def vanity_projects
  ...
  Movie.select(:id,:title,:name).joins(:actors).where('actors.id = movies.director_id AND ord = 1')
    # Movie.select(:id,:title,:name).joins(:actors).where(castings: {ord: 1}).where(actors: {id: :director_id})
end
